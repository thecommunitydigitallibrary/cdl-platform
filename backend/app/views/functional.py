...
def cache_search(query, search_id, index, communities, user_id, own_submissions=False):
	...
	if own_submissions or query == "":
		...
		if not page:
			_, hits = elastic_manager.search(query, list(communities.keys()), page=0, page_size=1000)
			pages = create_page(hits, communities)
...
def create_page(hits, communities):
	...
	for i, hit in enumerate(hits):
		...
		if hit["_source"]["highlighted_text"]:
		...
		result["explanation"] = hit["_source"]["explanation"]
