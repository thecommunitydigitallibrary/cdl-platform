      ...
      <>
        ...
        {isClient ? (
            <div className="graph" id="graph-outer-div" style={{ display: "flex" }}>
              ...
              {!!isModalOpen && (
                <Paper elevation={0} id="view-data-modal" style={{ marginTop: "6%" }}>
                  <Grid container display={"flex"} direction={"column"} justifyContent={"center"} alignItems={"center"}>
                      <Grid item sx={{ textAlign: 'center' }}>
                          <h5>Hashtag: {submissions["md_name"].split("/")[1]} | Topic: {submissions["md_name"].split("/")[2]} | Meta-descriptor: {submissions["md_name"].split("/")[3]}</h5>
                      </Grid>
                    <Grid container borderTop={"1px solid lightgray"} display={"flex"}>
                      <InfiniteScroll
